...
class Spectrum:
    ...
    def __init__(self, path, saturation=None, skip_field=1, skip_freq=1, derivative_divide=True , derivative=False,warning=True,modulation_amp=1):
        ...
        :param saturation: the no. of san where the Kittel mode saturates (Hc2)
        ...
        :derivative_divide and derivative: if False and derivative also false then the real data difference will be shown. If using the builtin plot function will probably need to readjust the vmin and vmax values to get a better plot.
        :param warning: if set to true will warn the user about not setting the no. of scan where the Kittel mode saturates
        :modulation amp: number of line cuts around the main line cut foe averaging to reduce noise, (kind of convolution size for averaging)
        """
    ...
    def get(self, skip_field=1, skip_freq=1):
        ...
        after background has been subtracted if derivatie_divide is set to False.
        """ 
    ...
    def scatter_plot(self, field_cuts_list, frequencies_list, frequencies_end_list = None, colors = None, denoise = False, save_name = None, v_min=-0.005, v_max=0.005,c_map="PuOr",style="Beam",pic_size=240,f_lim=None,s_lim=None,nom_locator=50):
        """
        Creates a figure of the FMR spectrum with points corresponding to the resonance frequencies of the fitted anti-lorentzian peaks.
        
        :param field_cuts: a list/tuple of row numbers (i.e. [arange(33, 36, 1), arange(40, 45), ...]). These elements five the row number range in which the given instance of fitting
        will occur
